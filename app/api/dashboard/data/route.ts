...
async function getFreshAccessToken(): Promise<string> {
  const tokenResponse = await fetch("https://accounts.zoho.com.au/oauth/v2/token", {
    method: "POST",
  ...
  if (!tokenData.access_token) throw new Error("Unable to refresh access token");
...
export async function GET() {
  try {
    ...
    const accountRes = await fetch(
      `https://www.zohoapis.com.au/crm/v6/Accounts/${TEST_ACCOUNT_ID}`,
    ...
    const dealRes = await fetch(
      `https://www.zohoapis.com.au/crm/v6/Deals/search?criteria=(Account_Name.id:equals:${TEST_ACCOUNT_ID})AND(Clearance_Type:equals:Corporate%20Clearance)&fields=Deal_Name,Stage,Amount,Created_Time&per_page=1`,
      { headers: { Authorization: `Zoho-oauthtoken ${accessToken}` } }
    );
