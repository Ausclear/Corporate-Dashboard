...
export default function RegisterPage() {
  ...
  const valid = acct.trim().length >= 4 && pinsMatch;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/auth/register", {
        ...
        body: JSON.stringify({ account_number: acct, pin }),
      });
  ...
  const C = isDark ? {
    ...
    gold: "#c9a84c", btnOff: "#1a1a22", btnOffText: "#3a3a42",
    errBg: "rgba(201,90,90,0.08)", errBorder: "rgba(201,90,90,0.2)", errText: "#c97a7a",
    green: "#5cb87a", greenBorder: "rgba(92,184,122,0.3)", greenBg: "rgba(92,184,122,0.1)",
    ...
    gold: "#9a7530", btnOff: "#e8e8f0", btnOffText: "#a0a0a8",
    errBg: "rgba(201,90,90,0.06)", errBorder: "rgba(201,90,90,0.15)", errText: "#b85050",
    green: "#2d8a4e", greenBorder: "rgba(45,138,78,0.3)", greenBg: "rgba(45,138,78,0.08)",
      ...
      <div style={{ width:"100%", maxWidth:420, position:"relative", zIndex:1 }}>
        ...
        <form onSubmit={handleSubmit} style={{ background:C.card, border:`1px solid ${C.cardBorder}`, borderRadius:16, padding:"32px 28px", boxShadow:C.shadow, borderTop:`2px solid ${C.gold}` }}>
          {success ? (
            ...
            <>
              <h2 style={{ fontSize:18, fontWeight:700, color:C.text, marginBottom:8 }}>Register</h2>
              <p style={{ fontSize:12, color:C.muted, marginBottom:20 }}>Enter your AusClear account number and choose a 6-digit PIN to secure your dashboard.</p>
              ...
              <div style={{ marginBottom:20 }}>
                <label style={{ display:"block", fontSize:10, color:C.label, marginBottom:8, textTransform:"uppercase", letterSpacing:"0.14em", fontWeight:600 }}>Create 6-Digit PIN</label>
                <div style={{ display:"flex", gap:8, justifyContent:"center" }}>
                  {[0,1,2,3,4,5].map(i => (
                    <input key={i} id={`pin-${i}`} type="text" inputMode="numeric" maxLength={1}
                      value={pin[i] || ""}
                      onChange={e => { const v = e.target.value.replace(/\D/g,""); if(!v&&pin[i]){setPin(pin.slice(0,i)+pin.slice(i+1));return;} if(!v)return; setPin(pin.slice(0,i)+v[0]+pin.slice(i+1)); if(i<5)document.getElementById(`pin-${i+1}`)?.focus(); }}
                      onKeyDown={e => { if(e.key==="Backspace"&&!pin[i]&&i>0)document.getElementById(`pin-${i-1}`)?.focus(); }}
                      style={{ width:48, height:56, textAlign:"center", fontSize:22, fontWeight:700, background:C.input, border:`1px solid ${pin[i]?C.inputFocus:C.inputBorder}`, borderRadius:8, color:C.text, outline:"none", boxSizing:"border-box", transition:"all 0.2s", WebkitTextFillColor:C.text }} />
                  ))}
                </div>
              </div>

              <div style={{ marginBottom:24 }}>
                <label style={{ display:"block", fontSize:10, color:C.label, marginBottom:8, textTransform:"uppercase", letterSpacing:"0.14em", fontWeight:600 }}>Confirm PIN</label>
                <div style={{ display:"flex", gap:8, justifyContent:"center" }}>
                  {[0,1,2,3,4,5].map(i => (
                    <input key={i} id={`cpin-${i}`} type="text" inputMode="numeric" maxLength={1}
                      value={confirmPin[i] || ""}
                      onChange={e => { const v = e.target.value.replace(/\D/g,""); if(!v&&confirmPin[i]){setConfirmPin(confirmPin.slice(0,i)+confirmPin.slice(i+1));return;} if(!v)return; setConfirmPin(confirmPin.slice(0,i)+v[0]+confirmPin.slice(i+1)); if(i<5)document.getElementById(`cpin-${i+1}`)?.focus(); }}
                      onKeyDown={e => { if(e.key==="Backspace"&&!confirmPin[i]&&i>0)document.getElementById(`cpin-${i-1}`)?.focus(); }}
                      style={{ width:48, height:56, textAlign:"center", fontSize:22, fontWeight:700, background:C.input, border:`1px solid ${confirmPin[i] ? (confirmPin.length===6 ? (pinsMatch?"rgba(92,184,122,0.5)":"rgba(201,90,90,0.5)") : C.inputFocus) : C.inputBorder}`, borderRadius:8, color:C.text, outline:"none", boxSizing:"border-box", transition:"all 0.2s", WebkitTextFillColor:C.text }} />
                  ))}
                </div>
              ...
              <p style={{ textAlign:"center", marginTop:20, fontSize:12, color:C.muted }}>
                Already registered? <a href="/login" style={{ color:C.gold, textDecoration:"none", fontWeight:500 }}>Sign In →</a>
              </p>
            </>
          )}
        </form>
