...
const LIGHT = {
  bg: "#f4f5f7", card: "#ffffff", card2: "#f8f9fa",
  line: "#e2e4e9", gold: "#2563b0", goldDim: "rgba(37,99,176,0.08)",
  text: "#1a1a2e", sub: "#3a3a4e", muted: "#6b6b7b", dim: "#9a9aaa",
...
type Props = { company: Co | undefined; personnel: P[]; batches: Batch[]; isMobile: boolean; theme: "dark" | "light" };
...
/* ═══ ANALYTICS TAB ═══ */
export default function AnalyticsTab({ company: co, personnel: ppl, batches: allBatches, isMobile: mob, theme }: Props) {
  const C = theme === "dark" ? DARK : LIGHT;
  const CLR_C: Record<string, string> = { Baseline: C.green, NV1: C.blue, NV2: C.amber };
  const REQ_C: Record<string, string> = { New: "#3a76b0", Upgrade: "#d4935c", Transfer: C.purple };

  const TT = ({ active, payload, label, cur }: any) => {
    if (!active || !payload?.length) return null;
    return (
      <div style={{ background: C.card2, border: `1px solid ${C.line}`, borderRadius: 6, padding: "8px 12px", fontSize: 11, color: C.text, boxShadow: "0 8px 24px rgba(0,0,0,.3)" }}>
  ...
  const PipelineTT = ({ active, payload, label }: any) => {
    if (!active || !payload?.length) return null;
    const stage = pipeline.find(p => p.stage === label);
    return (
      <div style={{ background: C.card2, border: `1px solid ${C.line}`, borderRadius: 6, padding: "8px 12px", fontSize: 11, color: C.text, boxShadow: "0 8px 24px rgba(0,0,0,.3)", maxWidth: 220 }}>
        <div style={{ color: C.muted, marginBottom: 4, fontWeight: 600, fontSize: 10 }}>{label}</div>
        <div style={{ fontWeight: 600, marginBottom: stage?.names?.length ? 4 : 0 }}>{payload[0]?.value} {payload[0]?.value === 1 ? "employee" : "employees"}</div>
        {stage?.names?.map((n: string, i: number) => (
          <div key={i} style={{ display: "flex", alignItems: "center", gap: 5, marginTop: 2 }}>
            ...
            <span style={{ color: C.sub, fontSize: 11 }}>{n}</span>
          </div>
        ))}
      </div>
    );
  ...
  const Gauge = ({ value, max, label, color, size = 80 }: { value: number; max: number; label: string; color: string; size?: number }) => {
    ...
    const r = size * 0.38, sw = Math.max(4, size * 0.065), cx = size / 2, cy = size / 2;
    const endFill = 225 + (pct / 100) * 270;
    return (
      <div style={{ textAlign: "center" }}>
        <div style={{ position: "relative", width: size, height: size, margin: "0 auto" }}>
          <svg width={size} height={size}>
            <path d={arcPath(cx, cy, r, 225, 495)} fill="none" stroke={C.line} strokeWidth={sw} strokeLinecap="round" />
            {pct > 1 && <path d={arcPath(cx, cy, r, 225, endFill)} fill="none" stroke={color} strokeWidth={sw} strokeLinecap="round" />}
          </svg>
  ...
  const SH = ({ title, icon, count }: { title: string; icon: string; count?: string | number }) => (
  ...
  const CardW = ({ children, style: s }: { children: React.ReactNode; style?: React.CSSProperties }) => (
  ...
  const pipeline = useMemo(() => {
    const c: Record<string, { stage: string; count: number; o: number; names: string[] }> = {};
    ALL_STAGES.forEach((f, i) => { c[f] = { stage: f, count: 0, o: i, names: [] }; });
    ppl.forEach(e => {
      const s = STAGE_ORD[e.stage];
      if (s) { c[s.f].count++; c[s.f].names.push(e.employee_name); }
    });
  ...
  const progress = useMemo(() => {
    return ppl.map(e => {
      ...
      const cl = e.clearance_type?.includes("NV2") ? "NV2" : e.clearance_type?.includes("NV1") ? "NV1" : "Baseline";
      const reqFee = e.clearance_request_type === "Transfer" ? 260 : e.clearance_request_type === "Upgrade" ? 360 : 410;
      const agsFee = e.clearance_request_type === "Transfer" ? 0 : (cl === "NV2" ? 3790 : cl === "NV1" ? 1897 : 892);
      return {
        ...e, pct, friendly: s?.f || e.stage, cl,
        cost: reqFee + 1400 + agsFee,
      };
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      {subView === "overview" && <>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: mob ? 8 : 12 }}>
          <KPI label="Total Employees" value={ppl.length} sub={`${batches.length} group${batches.length !== 1 ? "es" : ""}`} color={C.blue} spark={sparkE} sparkColor={C.blue} />
        ...
        <div style={{ display: mob ? "flex" : "grid", gridTemplateColumns: "1fr 260px", flexDirection: "column", gap: 12 }}>
          <CardW style={{ padding: mob ? 12 : 16 }}>
            <SH title="Status Distribution" icon="▦" count={ppl.length}  />
            <ResponsiveContainer width="100%" height={mob ? 220 : 280}>
              <BarChart data={pipeline} layout="vertical" margin={{ left: 0, right: 16, top: 0, bottom: 0 }}>
                ...
                <Tooltip content={<PipelineTT />} cursor={{ fill: C.goldDim }} />
                <Bar dataKey="count" name="Employees" radius={[0, 4, 4, 0]} maxBarSize={16}>
                  {pipeline.map((e, i) => {
                    const STAGE_COLORS = ["#6b9fd4","#4a90c4","#3a82b8","#2d8a4e","#5cb87a","#c07c2a","#d4935c","#9a7530","#c9a84c","#2d8a4e"];
                    return <Cell key={i} fill={e.stage === "Security Clearance Granted" ? "#2d8a4e" : (STAGE_COLORS[i % STAGE_COLORS.length])} fillOpacity={e.count > 0 ? 0.85 : 0.15} />;
                  })}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
        ...
        {/* Staff by Stage — instant visibility of who's where */}
        <CardW style={{ padding: mob ? 12 : 16 }}>
          <SH title="Staff by Stage" icon="👥" count={ppl.length} />
          <div style={{ display: "flex", gap: 10, overflowX: "auto", paddingBottom: 6 }}>
            {(() => {
              const grouped: Record<string, typeof ppl> = {};
              ppl.forEach(p => {
                const stage = p.stage?.replace(/([a-z])([A-Z])/g, "$1 $2") || "Pending";
                if (!grouped[stage]) grouped[stage] = [];
                grouped[stage].push(p);
              });
              const STAGE_COLS: Record<string, string> = {
                "Sponsorship Created": "#6b9fd4", "Onboard Employee for ESC": "#4a90c4",
                "ESC Pending": "#3a82b8", "ESC Completed": "#2d7aaa", "ESC Approved": "#2d8a4e",
                "AGSVA Portal Access": "#5cb87a", "AGSVA Portal": "#c07c2a",
                "AGSVA Clearance Onboard": "#d4935c", "AGSVA Clearance Pending": "#c9a84c",
                "AGSVA Clearance Granted": "#2d8a4e",
              };
              return Object.entries(grouped).map(([stage, employees]) => (
                <div key={stage} style={{ minWidth: mob ? 200 : 220, background: C.card2, border: `1px solid ${C.line}`, borderRadius: 8, padding: 12, flexShrink: 0, borderTop: `3px solid ${STAGE_COLS[stage] || C.muted}` }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: STAGE_COLS[stage] || C.muted, marginBottom: 8, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <span>{stage}</span>
                    <span style={{ background: `${STAGE_COLS[stage] || C.muted}18`, padding: "2px 8px", borderRadius: 10, fontSize: 10 }}>{employees.length}</span>
                  </div>
                  <div style={{ display: "flex", flexDirection: "column", gap: 6, maxHeight: 120, overflowY: "auto" }}>
                    {employees.map(e => (
                      <div key={e.id || e.employee_name} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12 }}>
                        <div style={{ width: 24, height: 24, borderRadius: "50%", background: `${STAGE_COLS[stage] || C.muted}18`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 9, fontWeight: 700, color: STAGE_COLS[stage] || C.muted, flexShrink: 0 }}>
                          {e.employee_name.split(" ").map((w: string) => w[0]).join("").slice(0, 2)}
                        </div>
                        <div style={{ minWidth: 0 }}>
                          <div style={{ fontWeight: 600, color: C.text, fontSize: 12, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{e.employee_name}</div>
                          <div style={{ fontSize: 10, color: C.dim }}>{e.clearance_type}</div>
                        </div>
                      </div>
                    ))}
                  </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: mob ? "1fr 1fr" : "200px 200px 1fr", gap: 12 }}>
          ...
          <CardW style={{ padding: mob ? 12 : 16, gridColumn: mob ? "1 / -1" : "auto" }}>
            <SH title="Group Timeline" icon="▥" count={`${batches.length} group${batches.length !== 1 ? "es" : ""}`}  />
      ...
      {subView === "financial" && <>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: mob ? 8 : 12 }}>
          ...
          <KPI label="Application Fees" value={fmt(co?.total_application_fees || 0)} sub={co?.promotional_offer ? `$410 − ${co.promotional_offer}` : `${ppl.length} × $410`} color={C.amber} />
          <KPI label="Sponsorship Fees" value={fmt(co?.total_sponsorship_fees || 0)} sub="$1,400 per employee" color={C.blue} />
        ...
        <div style={{ display: mob ? "flex" : "grid", gridTemplateColumns: "280px 1fr", flexDirection: "column", gap: 12 }}>
          ...
          <CardW style={{ padding: mob ? 12 : 16 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8, flexWrap: "wrap", gap: 6 }}>
              <SH title="Investment by Group" icon="▤"  />
        ...
        {/* Batch cost table */}
        <CardW style={{ padding: mob ? 12 : 16 }}>
          <SH title="Cost per Group" icon="▧" count={batches.length}  />
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead>
                <tr>
                  {["Group", "Employees", "Application", "Sponsorship", "AGSVA", "Total"].map(h => (
      ...
      {subView === "personnel" && <>
        <CardW style={{ padding: mob ? 12 : 16 }}>
          <SH title="Employee Clearance Progress" icon="◉" count={ppl.length}  />
          {mob ? (
                  ...
                  <div key={e.id} onClick={() => setExpandedEmp(exp ? null : e.id)}
                    ...
                    {exp && (
                      <div style={{ marginTop: 10, paddingTop: 10, borderTop: `1px solid ${C.line}`, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6, fontSize: 11 }}>
                        ...
                        <div><span style={{ color: C.dim }}>Group</span><div style={{ color: C.sub }}>{e.batch_date || "—"}</div></div>
            ...
            <div style={{ overflowX: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead>
                  <tr>
                    {["Employee", "Clearance", "Request", "Group", "Stage", "Progress", "Cost"].map(h => (
                      <th key={h} style={{ textAlign: "left", padding: "8px 12px", borderBottom: `1px solid ${C.line}`, fontSize: 9, color: C.dim, textTransform: "uppercase", letterSpacing: 1, fontWeight: 700 }}>{h}</th>
                    ))}
                  </tr>
                </thead>
