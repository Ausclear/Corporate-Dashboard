...
const DEFAULT_STAGES = Object.keys(STAGE_LABELS).slice(0, 10);
...
function ChevronPipeline({ stages, activeStage }: { stages: string[]; activeStage: string }) {
  ...
  return (
    <div ref={ref} onMouseDown={onMouseDown} onMouseMove={onMouseMove} onMouseUp={onMouseUp} onMouseLeave={onMouseUp}
      style={{ overflowX: "auto", cursor: "grab", userSelect: "none", scrollbarWidth: "none", msOverflowStyle: "none" as any }}>
...
export default function Dashboard() {
  ...
  const CORP_STAGES  = co?.pipeline_stages || Object.keys(STAGE_LABELS).slice(0, 13);
        ...
        {ppl.slice(0, 4).map((p, i) => (
          <div key={p.id} style={{ display: "flex", gap: 12, alignItems: "center", padding: "11px 18px",
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 11, color: "#7a7a82", marginTop: 2 }}>{label(p.stage) || "—"}</div>
            </div>
  ...
  const Pipeline = () => {
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        {batches.length === 0 && (
          <div style={{ background: "#111318", border: "1px solid #252b38", padding: 40, textAlign: "center" as const, color: "#4a4a52" }}>No batches found.</div>
        )}
        ...
        {batches.map((batch) => {
          ...
          return (
            <div key={batch.id} style={{ background: "#111318", border: "1px solid #252b38", borderTop: "2px solid #c9a84c" }}>
              <div onClick={() => setOpenBatch(isOpen ? null : batch.id)}
                ...
                <div>
                  ...
                  <div style={{ fontSize: 11, color: "#7a7a82", display: "flex", gap: 12, flexWrap: "wrap" as const }}>
                    <span>Stage: <span style={{ color: "#c9a84c", fontWeight: 600 }}>{label(batch.stage)}</span></span>
                  ...
                  {batch.nominees.map((p, pi) => {
                    ...
                    return (
                      <div key={p.id} style={{ borderBottom: pi < batch.nominees.length - 1 ? "1px solid #1a1f2c" : "none" }}>
                        <div onClick={() => setOpenEmp(empOpen ? null : p.id)}
          ...
          return (
            <div key={p.id} style={{ borderBottom: i < ppl.length - 1 ? "1px solid #252b38" : "none" }}>
              <div onClick={() => setExpanded(open ? null : p.id)}
                style={{ display: "flex", gap: 14, alignItems: "center", padding: "14px 18px", cursor: "pointer", background: open ? "#161922" : "transparent" }}
                ...
                <div style={{ display: "flex", gap: 6, alignItems: "center", flexShrink: 0 }}>
                  ...
                  <span style={{ color: "#4a4a52", fontSize: 16, display: "inline-block", transform: open ? "rotate(90deg)" : "rotate(0deg)", transition: "transform 0.2s" }}>›</span>
                </div>
              </div>
              {open && (
                <div style={{ background: "#161922", borderTop: "1px solid #252b38", padding: "16px 18px 20px 40px" }}>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px 32px" }}>
                    {[
                      ...
                      <div key={ri}>
                        <div style={{ fontSize: 10, color: "#c9a84c", textTransform: "uppercase" as const, letterSpacing: "0.12em", fontWeight: 700, marginBottom: 3 }}>{row.label}</div>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      <div style={{ background: "#111318", border: "1px solid #252b38" }}>
        {[
          ...
          <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 18px", borderBottom: i < arr.length - 1 ? "1px solid #252b38" : "none" }}>
            <div>
  ...
  if (loading) return (
    <div style={{ minHeight: "100vh", background: "#07070a", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ textAlign: "center" as const }}>
        <div style={{ width: 32, height: 32, border: "2px solid #252b38", borderTopColor: "#c9a84c", borderRadius: "50%", animation: "spin 0.8s linear infinite", margin: "0 auto 12px" }} />
