...
export default function Dashboard() {
  ...
  const Overview = () => (
    <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
      {isMobile && (
        <div style={{ background:C.card, border:`1px solid ${C.line}`, borderTop:`2px solid ${C.gold}`, padding:"18px 20px" }}>
      ...
      <div style={{ background:C.card, border:`1px solid ${C.line}` }}>
        <div style={{ padding:"12px 18px", borderBottom:`1px solid ${C.line}`, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          <span style={{ fontSize:13, fontWeight:600, color:C.text }}>Current Batch Stage</span>
  ...
  const Batches = () => (
    <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
      <div>
        ...
        <div style={{ fontSize:20, fontWeight:700, color:C.text }}>{co?.company_name} · <span style={{ fontFamily:"monospace", color:C.gold }}>{co?.account_number}</span></div>
      </div>
      <div style={{ background:C.card, border:`1px solid ${C.line}`, padding:"14px 18px" }}>
      ...
      {batches.map((batch) => {
        ...
        return (
          <div key={batch.id} style={{ background:C.card, border:`1px solid ${C.line}`, borderTop:`2px solid ${C.gold}` }}>
                ...
                <div style={{ background:C.card2, borderTop:`1px solid ${C.line}`, padding:"16px 18px 20px 40px" }}>
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"12px 32px" }}>
                    {[
                      ...
                      <div key={ri}>
                        <div style={{ fontSize:10, color:C.gold, textTransform:"uppercase" as const, letterSpacing:"0.12em", fontWeight:700, marginBottom:3 }}>{row.label}</div>
  ...
  const Financials = () => (
    <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
      <div style={{ background:C.card, border:`1px solid ${C.line}`, borderTop:`2px solid ${C.gold}`, padding:"20px 22px" }}>
  ...
  return (
    <div style={{ minHeight:"100vh", background:C.bg, color:C.text, fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif", display:"flex", flexDirection:"column" }}>
      ...
      <div style={{ position:"sticky", top:0, zIndex:50, background:"rgba(13,16,24,0.97)", backdropFilter:"blur(12px)", borderBottom:`1px solid ${C.line}`, height:56, padding:"0 20px", flexShrink:0, display:"flex", alignItems:"center", justifyContent:"space-between" }}>
          ...
          <div style={{ position:"relative" }}>
            ...
            {menuOpen && (
              <>
                ...
                <div style={{ position:"absolute", right:0, top:38, background:C.card, border:`1px solid ${C.line}`, zIndex:50, minWidth:180, boxShadow:"0 8px 24px rgba(0,0,0,0.6)" }}>
                  {TABS.map(t => (
                    <button key={t.key} onClick={() => { setTab(t.key); setMenuOpen(false); }}
                      style={{ display:"block", width:"100%", padding:"13px 18px", border:"none", borderBottom:`1px solid ${C.line}`, background:tab===t.key?"rgba(201,168,76,0.08)":"transparent", color:tab===t.key?C.gold:C.text, fontSize:13, cursor:"pointer", textAlign:"left" as const }}>
                      {t.label}
                    </button>
                  ))}
      ...
      <div style={{ flex:1, display:"flex", minHeight:0 }}>
        {!isMobile && (
          <div style={{ width:220, flexShrink:0, background:C.side, borderRight:`1px solid ${C.line}`, position:"sticky", top:56, height:"calc(100vh - 56px)", display:"flex", flexDirection:"column", overflowY:"auto" }}>
            <div style={{ padding:"20px 18px 16px", borderBottom:`1px solid ${C.line}` }}>
              ...
              <div style={{ fontSize:11, color:C.muted, marginTop:2 }}>Acct: <span style={{ color:C.gold, fontFamily:"monospace" }}>{co?.account_number || "—"}</span></div>
              <div style={{ marginTop:10 }}>
                <span style={{ fontSize:10, fontWeight:700, color:C.green, background:"rgba(92,184,122,0.12)", border:"1px solid rgba(92,184,122,0.3)", padding:"3px 10px", borderRadius:3 }}>Active</span>
              </div>
            </div>
            ...
            <nav style={{ padding:"8px 0", flex:1 }}>
              {TABS.map(t => (
                <button key={t.key} onClick={() => setTab(t.key)}
                  style={{ display:"block", width:"100%", padding:"11px 18px", border:"none", borderLeft:tab===t.key?`3px solid ${C.gold}`:"3px solid transparent", background:tab===t.key?"rgba(201,168,76,0.06)":"transparent", color:tab===t.key?C.gold:C.muted, fontSize:13, fontWeight:tab===t.key?600:400, cursor:"pointer", textAlign:"left" as const }}>
                  {t.label}
                </button>
              ))}
            </nav>
